...
| UI & helpers					           | These helper functions manage user input, display the puzzle in a 3×3 layout, and let the user enter a puzzle and choose a heuristic for solving.                                                                                                         | format_state(state)<br/>ask_int(...)<br/> parse_state_from_input(...)<br/>choose_heuristic(...) | User entries such as numbers for the puzzle, heuristic selection, and numeric values through the console. | Formatted puzzle display, validated puzzle state, selected heuristic function, and confirmed numeric input.
| UI actions				              | Lets the user solve one 8-puzzle by choosing a puzzle type and heuristic, then shows the moves, nodes, time, and optional solution steps.                                                                                                                 | ui_solve_once()<br/>ui_benchmark()                                                             |User choices for puzzle type, heuristic, and showing steps. | Displays puzzle, solvability, results, and solution steps.
| Experiment/Benchmark				    | Runs tests comparing Hamming and Manhattan heuristics on random puzzles, then shows average performance results in a table.                                                                                                                                 | run_experiment(...)<br/>summarize(...)<br/>print_table(...)                                    | User choices for trials, scramble steps, and seed. | Shows average nodes, runtime, and solution length for both heuristics.
